...
import com.aman.vanish.core.util.ext.isHttpUrl
import com.aman.vanish.core.util.ext.restartApplication
import com.aman.vanish.details.ui.pager.EmptyMangaReason
import org.koitharu.kotatsu.parsers.exception.AuthRequiredException
import org.koitharu.kotatsu.parsers.exception.NotFoundException
import org.koitharu.kotatsu.parsers.model.Manga
import org.koitharu.kotatsu.parsers.model.MangaSource
import com.aman.vanish.scrobbling.common.domain.ScrobblerAuthRequiredException
import com.aman.vanish.scrobbling.common.ui.ScrobblerAuthHelper
...
class ExceptionResolver private constructor(
    ...
    private val manualCaptchaContract = host.registerForActivityResult(object : ActivityResultContract<CloudFlareProtectedException, Boolean>() {
        override fun createIntent(context: Context, input: CloudFlareProtectedException): Intent {
            return ManualCaptchaActivity.newIntent(context, input.url, input.source)
        }

        override fun parseResult(resultCode: Int, intent: Intent?): Boolean {
            return resultCode == ManualCaptchaActivity.RESULT_SOLVED
        }
    }) {
        handleActivityResult(ManualCaptchaActivity.TAG, it)
    }
    ...
    private suspend fun resolveCF(e: CloudFlareProtectedException, tryAutoResolve: Boolean): Boolean {
        val userChoice = suspendCoroutine<Boolean> { cont ->
            ...
            com.google.android.material.dialog.MaterialAlertDialogBuilder(context)
                .setTitle(R.string.captcha_required_title)
                .setMessage(R.string.captcha_required_message)
                .setPositiveButton(R.string.captcha_solve) { _, _ ->
                    cont.resume(true)
                }
                .setNegativeButton(R.string.captcha_cancel) { _, _ ->
                    cont.resume(false)
                }
                .setCancelable(false)
                .show()
        ...
        if (!userChoice) {
            val context = host.context
            if (context != null) {
                Toast.makeText(context, R.string.captcha_cancelled, Toast.LENGTH_SHORT).show()
            }
            return false
        }

        return suspendCoroutine { cont ->
            continuations[ManualCaptchaActivity.TAG] = cont
            manualCaptchaContract.launch(e)
        }
    }
