...
import com.aman.vanish.core.network.proxy.ProxyProvider
import com.aman.vanish.core.parser.MangaRepository
import com.aman.vanish.core.parser.ParserMangaRepository
import com.aman.vanish.core.ui.util.ForegroundActivityHolder
import com.aman.vanish.core.util.ext.configureForParser
import com.aman.vanish.core.util.ext.prepareDetachedParserViewport
import com.aman.vanish.core.util.ext.printStackTraceDebug
import com.aman.vanish.core.util.ext.sanitizeHeaderValue
...
class WebViewExecutor @Inject constructor(
	...
	suspend fun tryResolveCaptcha(blockedUrl: String, sourceUserAgent: String?, timeout: Long = 60_000L): Boolean = mutex.withLock {
		withContext(Dispatchers.Main.immediate) {
			val webView = obtainWebView()
			if (sourceUserAgent != null) {
				webView.settings.userAgentString = sourceUserAgent
			}
			val result = runCatching {
				val oldClearance = CloudFlareHelper.getClearanceCookie(blockedUrl)

				withTimeout(timeout) {
					suspendCancellableCoroutine { continuation ->
						val continuationClient = CaptchaContinuationClient(
							targetUrl = blockedUrl,
							oldClearance = oldClearance,
							cookieJar = cookieJar,
							continuation = continuation
						)
						webView.webViewClient = continuationClient
						webView.loadUrl(blockedUrl)
					}
				}
			}.getOrDefault(false)
	...
	@MainThread
	fun getWebView(): WebView = obtainWebView()
	...
	private fun obtainWebView(): WebView = webViewCached?.get() ?: WebView(context).also { webView ->
		...
		webView.prepareDetachedParserViewport()
