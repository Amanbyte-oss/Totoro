...
import com.aman.vanish.backups.ui.periodical.PeriodicalBackupService
import com.aman.vanish.core.exceptions.resolve.SnackbarErrorObserver
import com.aman.vanish.core.nav.router
import com.aman.vanish.core.os.VoiceInputContract
import com.aman.vanish.core.prefs.AppSettings
import com.aman.vanish.core.prefs.NavItem
import com.aman.vanish.core.ui.BaseActivity
import com.aman.vanish.core.ui.util.FadingAppbarMediator
import com.aman.vanish.core.ui.util.MenuInvalidator
import com.aman.vanish.core.ui.widgets.FloatingBottomNavigationView
import com.aman.vanish.core.ui.widgets.SlidingBottomNavigationView
import com.aman.vanish.core.util.ext.consume
import com.aman.vanish.core.util.ext.end
import com.aman.vanish.core.util.ext.observe
import com.aman.vanish.core.util.ext.observeEvent
import com.aman.vanish.core.util.ext.printStackTraceDebug
import com.aman.vanish.core.util.ext.start
import com.aman.vanish.databinding.ActivityMainBinding
import com.aman.vanish.details.service.MangaPrefetchService
import com.aman.vanish.favourites.ui.container.FavouritesContainerFragment
import com.aman.vanish.history.ui.HistoryListFragment
import com.aman.vanish.local.ui.LocalIndexUpdateService
import com.aman.vanish.local.ui.LocalStorageCleanupWorker
import com.aman.vanish.main.ui.owners.AppBarOwner
import com.aman.vanish.main.ui.owners.BottomNavOwner
import org.koitharu.kotatsu.parsers.model.Manga
import com.aman.vanish.remotelist.ui.MangaSearchMenuProvider
import com.aman.vanish.search.ui.suggestion.SearchSuggestionItemCallback
import com.aman.vanish.search.ui.suggestion.SearchSuggestionListenerImpl
import com.aman.vanish.search.ui.suggestion.SearchSuggestionMenuProvider
import com.aman.vanish.search.ui.suggestion.SearchSuggestionViewModel
import com.aman.vanish.search.ui.suggestion.adapter.SearchSuggestionAdapter
...
class MainActivity : BaseActivity<ActivityMainBinding>(), AppBarOwner, BottomNavOwner,
	...
	@Inject
	lateinit var analyticsHelper: com.aman.vanish.core.analytics.AnalyticsHelper
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		// Example manual screen view logging (automatic tracking is also enabled in lifecycle callbacks)
		analyticsHelper.logScreenView("MainActivity", javaClass.name)

		// Example login logging
		analyticsHelper.logLogin("Google")

		// Debug mode toggle configuration
		analyticsHelper.setDebugAnalyticsEnabled(com.aman.vanish.BuildConfig.DEBUG)

		// Example user property setting (premium status)
		analyticsHelper.setUserPremiumStatus(isPremium = false)
		...
		addMenuProvider(MainMenuProvider(this, router, viewModel))
	...
	override fun onClick(v: View) {
		when (v.id) {
			R.id.fab, R.id.railFab -> {
				// Example button click event logging
				analyticsHelper.logButtonClick("open_last_reader_fab", "MainActivity")
				viewModel.openLastReader()
			}
		}
	}
	...
	/**
	 * Example helper to track a premium purchase and update premium user property.
	 */
	fun trackPremiumPurchaseDemo() {
		// Custom purchase event with item_id, value, and currency
		analyticsHelper.logPurchase(
			itemId = "premium_subscription_monthly",
			value = 4.99,
			currency = "USD"
		)
		// Update user property
		analyticsHelper.setUserPremiumStatus(isPremium = true)
	}
