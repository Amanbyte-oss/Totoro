package com.aman.vanish.explore.data
...
import com.aman.vanish.core.ui.util.ReversibleHandle
import com.aman.vanish.core.util.ext.flattenLatest
...
class MangaSourcesRepository @Inject constructor(
	...
	private suspend fun assimilateNewSources(): Boolean {
		...
		if (new.isEmpty()) {
			initializeDefaultPinnedSources()
			return false
		}
	...
	suspend fun initializeFirstLaunch() {
		...
		val flagKey = "default_sources_set_v2"
		if (prefs.getBoolean(flagKey, false)) {
			return
		}
		try {
			android.util.Log.i("DefaultSources", "Initializing default sources first-launch")
			val existingPinned = dao.findAllPinned()
			val existingEnabled = dao.findAllEnabledNames()
			if (existingPinned.isNotEmpty() || existingEnabled.isNotEmpty()) {
				android.util.Log.i("DefaultSources", "User already has pinned/enabled sources, skipping defaults")
				prefs.edit().putBoolean(flagKey, true).apply()
				return
			}
			
			// 1. Set default enabled sources (ALL sources EXCEPT Nude-Moon and MangaBuff)
			val excluded = setOf("NUDEMOON", "MANGABUFF")
			db.withTransaction {
				assimilateNewSources()
				for (s in allMangaSources) {
					val isExcluded = s.name in excluded
					dao.setEnabled(s.name, !isExcluded)
				}
				
				// 2. Pin the 6 default sources
				...
				for (sourceName in defaultPins) {
					dao.setPinned(sourceName, true)
					dao.setEnabled(sourceName, true)
				}
			}
			
			prefs.edit().putBoolean(flagKey, true).apply()
			android.util.Log.i("DefaultSources", "Default sources first-launch initialization complete")
		} catch (e: Exception) {
			android.util.Log.e("DefaultSources", "Failed to initialize default sources", e)
			prefs.edit().putBoolean(flagKey, true).apply()
		}
	}

	private suspend fun initializeDefaultPinnedSources() {
		initializeFirstLaunch()
	}

	suspend fun isSetupRequired(): Boolean {
		val prefs = PreferenceManager.getDefaultSharedPreferences(context)
		val setupRequired = !prefs.getBoolean("default_sources_set_v2", false)
		if (setupRequired) {
			val existingPinned = dao.findAllPinned()
			val existingEnabled = dao.findAllEnabledNames()
			if (existingPinned.isNotEmpty() || existingEnabled.isNotEmpty()) {
				prefs.edit().putBoolean("default_sources_set_v2", true).apply()
				return false
			}
		}
